...
const fetchProducts = async () => {
  ...
  products.value = await Promise.all(snapshot.docs.map(async (docSnap) => {
    const data = docSnap.data();
    const itemRef = doc(db, 'items', data.itemId);
    const itemSnap = await getDoc(itemRef);
    return { id: docSnap.id, ...data, item: itemSnap.data() || null };
  }));
};
...
const addProduct = async () => {
  ...
  await addDoc(collection(db, 'products'), { ...form.value, image: imageToUse });
  await fetchProducts();
